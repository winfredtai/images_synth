...
n_test_samples = 5     # TODO
n_ground_truths = 5     # each image has 5 ground truth captions
...
for i in range(len(image_names)):  # TODO
...
print("\nMean metrics (on every 5 ground_truth_captions):\n")
...
# maximum & average
max_BLEU = [max(bleu_scores[i:i+5]) for i in range(0, len(bleu_scores), 5)]
max_METEOR = [max(meteor_scores[i:i+5]) for i in range(0, len(meteor_scores), 5)]
max_CIDEr = avg_CIDEr  # since CIDEr is a single value
max_ROUGE_L = [max(rouge_l_scores[i:i+5]) for i in range(0, len(rouge_l_scores), 5)]
max_LLM_deepseekv3 = [max(llm_scores_deepseekv3[i:i+5]) for i in range(0, len(llm_scores_deepseekv3), 5)]
max_LLM_gpt41mini = [max(llm_scores_gpt41mini[i:i+5]) for i in range(0, len(llm_scores_gpt41mini), 5)]

mean_max_BLEU = sum(max_BLEU)/len(max_BLEU)
mean_max_METEOR = sum(max_METEOR)/len(max_METEOR)
mean_max_ROUGE_L = sum(max_ROUGE_L)/len(max_ROUGE_L)
# mean_max_CIDEr = avg_CIDEr  # since CIDEr is a single value
mean_max_LLM_deepseekv3 = sum(max_LLM_deepseekv3)/len(max_LLM_deepseekv3)
mean_max_LLM_gpt41mini = sum(max_LLM_gpt41mini)/len(max_LLM_gpt41mini)

print("\nMean Maximum metrics:\n")
print(f"Mean Maximum BLEU: {mean_max_BLEU:.3f}")
print(f"Mean Maximum METEOR: {mean_max_METEOR:.3f}")
print(f"Mean Maximum ROUGE-L: {mean_max_ROUGE_L:.3f}")
print(f"Mean Maximum LLM (deepseek-v3): {mean_max_LLM_deepseekv3:.3f}")
print(f"Mean Maximum LLM (gpt-4.1-mini): {mean_max_LLM_gpt41mini:.3f}")
...
# align with experiments in evaluate_Winfred.py
# random_indices = [654 * n_ground_truths, 114 * n_ground_truths, 25 * n_ground_truths, 759 * n_ground_truths]
...
# get_index from image_names
random_indices = [image_names.index(name) for name in sampled_image_names]
check_sampled_image_names = [image_names[i] for i in random_indices]
assert sampled_image_names == check_sampled_image_names

sampled_gt_captions = [gt_captions[i:i+n_ground_truths] for i in random_indices]
...
for i, (img_name, true_caption_list, pred_caption) in enumerate(zip(sampled_image_names, sampled_gt_captions, sampled_captions)):
    # plot
    if i >= n_samples:
        break
    ...
    # display caption
    true_text = "\n".join([f"TRUE-{j+1}: {cap}" for j, cap in enumerate(true_caption_list)])
    pred_text = f"PRED: {pred_caption}"
    axes[i][1].text(0.1, 0.3, true_text, fontsize=14, wrap=True)
    axes[i][1].text(0.1, 0.1, pred_text, fontsize=14, wrap=True)
    axes[i][1].axis('off')
    ...
    axes[i][1].axis('off')
    axes[i][1].set_title(f"BLEU: (avg) {np.mean(bleu_scores[i:i+5]):.2f} | (max) {np.max(bleu_scores[i:i+5]):.2f}"
                         f"\nMETEOR: (avg) {np.mean(meteor_scores[i:i+5]):.2f} | (max) {np.max(meteor_scores[i:i+5]):.2f}"
                         f"\nROUGE-L: (avg) {np.mean(rouge_l_scores[i:i+5]):.2f} | (max) {np.max(rouge_l_scores[i:i+5]):.2f}"
                         f"\nCIDEr: {avg_CIDEr:.2f}"
                         f"\ndeepseek-v3: (avg) {np.mean(llm_scores_deepseekv3[i:i+5]):.2f} | (max) {np.max(llm_scores_deepseekv3[i:i+5]):.2f}"
                         f"\ngpt-4.1-mini: (avg) {np.mean(llm_scores_gpt41mini[i:i+5]):.2f} | (max) {np.max(llm_scores_gpt41mini[i:i+5]):.2f}")
...
def save_results_to_json():
    results = []
    # for i in range(0, len(image_names), 5): # for every image, we have 5 ground truth captions
    for i in range(0, len(image_names), n_ground_truths):  # TODO
        record = {
            "image_name": image_names[i],
            "caption": captions[i],
            "ground_truth_caption": gt_captions[i:i+n_ground_truths],  # 5 ground truth captions
            "BLEU": bleu_scores[i:i+n_ground_truths],  # 5 BLEU scores
            "METEOR": meteor_scores[i:i+n_ground_truths],  # 5 METEOR scores
            "CIDEr_mean": cider_mean,
            "ROUGE_L": rouge_l_scores[i:i+n_ground_truths],  # 5 ROUGE-L scores
            "LLM_deepseekv3": llm_scores_deepseekv3[i:i+n_ground_truths],  # 5 LLM scores (deepseek-v3)
            "LLM_gpt41mini": llm_scores_gpt41mini[i:i+n_ground_truths]  # 5 LLM scores (gpt-4.1-mini)
        }
